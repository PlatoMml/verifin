...
- 持久化：账目类核心数据（交易、账户、分组、账本、分类、月度/分类预算）存 SQLite，由 [lib/data/](lib/data/) 的 `AppDatabase`（建表 + 版本迁移）与 `LedgerRepository`（按实体整表覆盖式读写）承载。controller 内存列表仍是读取源，写入时经 `_persistX` 路由到库（`_repository != null` 时走 SQLite，否则走 KV，测试与降级用）。异步 `VeriFinController.create()` 完成首启动 KV→SQLite 迁移（按实体独立标记，库已有数据则不覆盖）并清理已进库的冗余 KV 键。偏好类小数据（主题、触感、面板配置、资产排序/折叠、封面等）仍以 JSON 存入 `LocalKeyValueStore`，键形如 `verifin.theme.v1`。修改 `lib/data/` 表结构须提升 `AppDatabase.schemaVersion` 并在 `_onUpgrade` 写迁移。
