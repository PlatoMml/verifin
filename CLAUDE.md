...
- 持久化：账目类核心数据（交易、账户、分组、账本、分类、标签、月度/分类预算）**只**存 SQLite，由 [lib/data/](lib/data/) 的 `AppDatabase`（建表 + 版本迁移）与 `LedgerRepository`（接口，整表覆盖式读写）承载，生产实现 `SqliteLedgerRepository`。controller 内存列表仍是读取源，写入经 `_persistX` 落库。`VeriFinController.create()`（唯一构造入口，异步）从库载入；全新数据库首启动播种默认账本/账户/分组/分类（标签默认为空）。交易与标签为多对多，交易表 `tag_ids` 列存 JSON 数组，不建关联表；标签全局共享不分账本。图片附件存独立 `attachments` 表（按 `entry_id` 关联，`data_url` 存压缩 JPEG），不放 entries 表以避免整表覆盖写入放大；删除交易/账户/账本时级联清理附件。没有 KV↔SQLite 迁移，也没有 KV 回退——账目类只认库。偏好类小数据（主题、触感、面板配置、资产排序/折叠、封面、活动账本、个人资料等）仍以 JSON 存入 `LocalKeyValueStore`，键形如 `verifin.theme.v1`。修改 `lib/data/` 表结构须提升 `AppDatabase.schemaVersion` 并在 `_onUpgrade` 写迁移。测试注入内存实现 `InMemoryLedgerRepository`（见 `test/support/`）。
- 多账本隔离：交易、账户、分组都带 `bookId`，controller 的 getter 按 `_activeBookId` 过滤后返回不可变列表；月份/分类预算的键带 `bookId:` 前缀（旧数据加载时自动迁移到默认账本）。
- 转账（`EntryType.transfer`）同时影响转出/转入账户余额，但不计入收支统计（聚合逻辑在 [lib/app/ledger_math.dart](lib/app/ledger_math.dart)）。转账可带手续费 `LedgerEntry.fee`，由转出账户承担（转出扣「金额+手续费」、转入只加金额）。支出可标记待报销（`reimbursable`）并被退款/报销回款冲抵（`refundedAmount`）：余额与所有统计都按 `LedgerEntry.netAmount`（金额−已冲抵）计算，回款回到原账户、不产生独立收入条目。

### 平台适配：条件导入三件套

Web/移动/测试差异统一用条件导出模式（`stub` + `if (dart.library.html)` + `if (dart.library.io)`）：

- [lib/local_storage/](lib/local_storage/)：`LocalKeyValueStore`（偏好类小数据）— io 用 `SharedPreferences`，测试用内存 stub。真实平台启动必须 `LocalKeyValueStore.create()`，**非 Web 平台绝不能落到内存实现**。
...
- `lib/app/attachment_picker_*.dart`：交易图片附件的拍照/相册选择 — io 用 `image_picker` 拍照或选图并压缩为 JPEG data URL（`image` 包在后台 isolate 缩放），stub（Web 与测试宿主）不支持（`attachmentPickingSupported=false`）；UI 组件 `lib/pages/attachments_editor.dart`。
