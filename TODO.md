...
- [ ] **7.1 全面屏手势冲突（Bug，先做）**：全面屏手势下退出动画与系统返回手势冲突。先排查复现条件（机型/手势导航模式/涉及的页面转场），再定方案。
...
- [ ] **7.3 按日自定义预算（可直接做）**：现有仅月度/分类预算，新增「按日」预算维度。清晰小增量，可立即推进。
- [ ] **7.4 AI 对话记账**：自然语言 → 结构化交易并自动分类。设计要点：
  - 用户自定义 **API Key + 请求基础地址（base URL）**（自带 Key、本地优先、数据不强制上云）。
  - 交互入口：把首页的记账按钮改造为「语音输入 / 打开聊天框」——用户可语音，也可手动输入自然语言（如「昨天打车 32」），AI 解析成交易草稿供确认落账。
  - 需处理：解析容错、落账前用户确认、隐私提示（发给第三方 AI 的数据边界）。
