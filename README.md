...
- 首页支出趋势卡可进入按月收支统计页，交易列表支持时间筛选和排序方式选择。
- 首页预算卡可进入月份预算设置页。
- 日历视图按日期展示支出/收入，并可点击具体日期查看当天交易。
- 交易详情页支持查看交易字段和删除交易，删除前会二次确认。
- 资产页支持添加账户、管理分组、查看账户详情。
- 个人页支持修改头像、昵称和简介，主题设置位于设置页。
- 交易记录、账户、分组、个人信息和主题偏好使用浏览器本地存储保存。
- 支持跟随系统、浅色、深色三种主题模式。
- 主交互色以 `#3498db` 蓝色为核心，青绿色仅作为辅助点缀。
- 当前 UI 已收敛为紧凑型移动端工具风格：图标式底部导航、小圆角卡片、较低字号和更高信息密度。
