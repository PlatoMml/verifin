...
- 首页 FAB 数字键盘**快速记账**，支持支出 / 收入 / 转账三种类型；可选**「无账户」**只记金额、不计入任何账户余额；
- **AI 对话记账**（可选）：把「记一笔」按钮设为 AI 模式，用一句话（如「昨天打车 32」）自动解析出类型 / 金额 / 分类 / 账户 / 备注草稿，确认后落账；自带 API Key + 请求地址（OpenAI 兼容），配置只存本机；
...
- **账单导入**：平台优先（先选来源再选文件）导入**支付宝**（CSV）、**微信**（xlsx）、**薄荷记账**（CSV）账单，及模板 / 钱迹 / 随手记 CSV；自动跳过还款、理财等「不计收支/中性交易」，各平台附「如何导出账单」引导；
