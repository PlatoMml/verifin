...
- 首页支出趋势卡按 7 天周期展示，并可进入收支统计页；交易列表支持时间周期切换、筛选和排序方式选择。
- 首页预算卡可进入月份预算设置页。
- 日历视图按日期展示支出/收入，并可点击具体日期查看当天交易。
- 交易详情页支持查看、编辑和删除交易，删除前会二次确认。
- 资产页支持添加账户、管理分组、查看账户详情、账户报告和账户交易筛选，净资产卡片可长按更换背景。
- 头像和资产背景支持选择本地图片后进入统一裁剪页。
- 账户删除会保护已有交易的账户，避免历史记录失去账户来源。
- 数据看板会按已保存支出记录聚合分类排行、日趋势和月度收支。
- 个人页支持修改头像、昵称和简介，主题设置位于设置页。
- 我的页支持账本管理，可新增、切换、重命名和删除非默认账本。
- 设置页支持初始化所有本地数据。
- 交易记录、账户、分组、个人信息、主题偏好和资产卡片背景使用浏览器本地存储保存。
- 支持跟随系统、浅色、深色三种主题模式。
