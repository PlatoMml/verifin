...
- 设置页支持触感反馈开关、检查 GitHub Release 更新、双重确认初始化所有本地数据，并支持导出/导入 JSON 本地备份；更新检查使用弹窗展示当前版本、最新版本和下载进度，Android 端默认导出到系统下载目录。
- Android 端支持下拉快捷开关“快速记账”，点击后进入应用并打开快速记账数字键盘。
- 账目类核心数据（交易记录、账户、分组、账本、分类、月份预算、分类预算）存入本地 SQLite（`sqflite`；Web 走 `sqflite_common_ffi_web` 的 wasm，测试走 ffi）；首次启动会把旧版 `SharedPreferences` 数据一次性迁移进库并清理冗余键。个人信息、主题偏好、资产视图偏好、资产分区折叠状态、账户排序、分区排序、首页与看板的面板配置和资产卡片背景等偏好类小数据仍存 `SharedPreferences`。进程被划掉后所有数据仍应保留。
- 支持跟随系统、浅色、深色三种主题模式。
- 日期和时间选择器使用中文本地化。
...
- `docs/dev/verifin-sample-backup.json`：可从设置页导入的测试备份数据，用于验证资产、预算、看板和交易列表。
