...
- 记账详情页支持支出、收入、转账类型，支持分类、账户、转入账户、单行备注、日期和时间。
...
- 交易记录、账户、分组、账本、分类、月份预算、分类预算、个人信息、主题偏好、资产视图偏好、资产分区折叠状态、账户排序、分区排序和资产卡片背景使用本地存储保存；Android 使用 `SharedPreferences`，进程被划掉后仍应保留。
- 支持跟随系统、浅色、深色三种主题模式。
- 日期和时间选择器使用中文本地化。
