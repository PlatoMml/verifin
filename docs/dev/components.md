...
| `CardNumberFields` | Widget | `common_widgets.dart` | 完整卡号 + 后四位输入组，含「后四位跟随卡号」开关（信用卡/储蓄卡录入用）；跟随初始态由 `initialCardLast4Follows` 反推 |
| `showCardNumberDialog` | Dialog 函数 | `sheets.dart` | 编辑完整卡号+后四位，返回 `({number, last4})?`（内部用 `CardNumberFields`，后四位以 `cardLast4Of` 归一化） |
...
| 信用类账户 | `credit_card.dart` | `nextDueDate` `daysUntilDue` `nextStatementDate` `currentBillingCycle` `usedCredit` `availableCredit` `billingCycleExpense`；卡号 `cardLast4Of` `initialCardLast4Follows`（在 `models.dart`） |
