...
| 名称 | 类型 | 位置 | 用途 / 关键点 |
|---|---|---|---|
| `CategoryPickerSheet` | Widget | `entry_sheets.dart` | **多级分类选择**（展开/收起层级树）；`allLabel` 非空加「全部」→ 返回 `categoryPickerAll`；`topLevelLabel` 加「移到顶级」→ 返回 `categoryPickerTopLevel`。当前经 `showModalBottomSheet` 打开（helper 化待办，见 `known-limitations.md`） |
...
| 名称 | 类型 | 位置 | 用途 / 关键点 |
|---|---|---|---|
| `NumberPadSheet` | Widget | `entry_sheets.dart` | **数字键盘**（四则算式 + 结果预览）；`allowNegative` `allowZero` `hapticsEnabled`。**输金额一律用它**，不要弹系统 TextField。当前经 `showModalBottomSheet` 打开（helper 化待办，见 `known-limitations.md`） |
