...
| CSV 导入范围 | 本应用 **CSV 模板**走自研 RFC-4180 解析器，入口用白名单严格校验表头（只允许模板列，`validateCsvTemplateHeader`），**不与第三方软件共用一套表头识别**；支付平台/记账软件账单按来源单独适配（支付宝/微信/薄荷/一木/Tally），各认自己的格式；微信 xlsx 用已有 `archive` 解压 + 正则解析 XML（不引入 `excel`/xlsx 依赖） | 不同软件解析逻辑解耦、各走独立入口才好维护，避免「一套通用识别硬套所有软件」误判（原「其他 CSV」把模板+钱迹+随手记混在一起，已下线通用识别）；微信只导出 xlsx，为体验直接在应用内解析而非要求用户转格式，复用 `archive` 与「手写 XML 正则」惯例避免重量级依赖 |
