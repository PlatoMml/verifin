...
| 导入预览再落库 | 解析（`controller.parsePlatformImport`，纯解析不落库）与落库（`controller.applyImportEntries`）拆开，中间插入 `ImportPreviewPage`：复用 `TransactionTile` 列出待导入交易，可排除 / 编辑（走 `EntryDetailPage.draft` 草稿模式，返回改后条目而不落库），确认后只创建被保留交易实际引用到的新账户/分类。全程零落库副作用（预览/编辑传「现有＋待新建」的合并账户/分类列表，`accountBalance` 对未落库账户天然返回其初始余额） | 直接入账无法纠错，账单常有脏数据/错分类；预览让用户先核对。`EntryDetailPage.draft` 做成通用「编辑一条交易并返回、不持久化」的模式，后续可复用。以后新增导入平台一律走此流程 |
