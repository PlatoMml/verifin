...
- **完整卡号与后四位**：`cardNumber`（完整，选填）与 `cardLast4`（后四位）都存。编辑页「后四位跟随卡号」开关（`CardNumberFields` 组件）打开时后四位自动取完整卡号末四位（`cardLast4Of`），关闭可手填；开关初始态由 `initialCardLast4Follows` 从数据反推、不额外持久化。列表/首页只显示后四位，详情页展示完整卡号并可复制。切换到不支持卡号的类型时清空 `cardNumber`/`cardLast4`。
